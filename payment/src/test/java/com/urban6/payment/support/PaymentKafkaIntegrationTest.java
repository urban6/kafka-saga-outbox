...
/**
 * payment 리스너를 <b>실제로 띄우고</b> 진짜 브로커로 커맨드를 밀어넣는 기반.
 * <p>
 * {@link PaymentIntegrationTest} 는 리스너를 끄고 유스케이스를 직접 부른다. 여기서 보는 건 그 앞단이다 —
 * 봉투 역직렬화, 모르는 커맨드, poison pill. <b>메시지가 브로커를 거쳐야</b> 재현되는 것들이다.
 * <p>
 * <b>포트가 18083 인 이유</b>: 스프링이 컨텍스트를 캐시해서 {@code PaymentIntegrationTest} 쪽(18082)이
...
public abstract class PaymentKafkaIntegrationTest {
	 ...
	 * {@code KafkaTemplate} 이나 봉투 객체를 쓰지 않는다. 키는 운영과 같은 {@code orderNo} 다.
	 */
