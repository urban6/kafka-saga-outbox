 ...
 * 변환 없이 맞춰볼 수 있다.
 * <p>
 * {@code READY} 는 이 프로젝트에 없다 — 결제창이 없어서 "승인 대기" 단계를 거치지 않는다.
 * {@code IN_PROGRESS} 는 "승인 요청은 보냈는데 결과를 모른다" 는 뜻이며, 지금은 그 상태로
 * 저장되는 경로가 없다(PG 응답을 받은 뒤에 저장하므로). in-doubt 를 다룰 때 생긴다.
 */
