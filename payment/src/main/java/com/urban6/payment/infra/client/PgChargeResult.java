 ...
 * <p>
 * 네 갈래를 가르는 기준은 하나다 — <b>돈이 빠졌는가.</b>
 * 승인/거절은 답을 알고, {@code RETRYABLE} 은 "안 빠진 게 확실",
 * {@code IN_DOUBT} 는 "모른다" 다. 모름을 거절로 접으면 이미 받은 돈에 보상이 돌고,
 * 재시도로 접으면 이중 결제가 난다. 그래서 셋이 아니라 넷이다.
 *
 * @param paymentKey  승인됐을 때만 값이 있다. PG 가 확인해준 식별자
 * @param failureCode 승인이 아닐 때의 근거 코드. {@code payment.failure_code} 에 그대로 남는다.
 *                    Toss 코드일 수도, 우리가 판정한 코드({@code NO_BILLING_KEY}, {@code PG_TIMEOUT})일 수도 있다
 */
public record PgChargeResult(
	...
	public enum Outcome {
		...
		REJECTED,
		/** 돈이 <b>안 빠진 게 확실</b>하고 재시도하면 달라질 수 있다. 재청구가 안전하다. */
		RETRYABLE,
		/** 빠졌는지 <b>모른다.</b> 재청구 금지 — 조회로 실제 상태를 먼저 확인해야 한다. */
		IN_DOUBT
	}
