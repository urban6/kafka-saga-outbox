...
public class ApprovePaymentService {
	...
	public Payment approve(String orderNo, BigDecimal amount) {
		return process(orderNo, amount, null);
	}

	/** Kafka 진입. 멱등 선점과 회신 적재까지 포함한다. */
	public Payment approve(UUID eventId, String orderNo, BigDecimal amount) {
		return process(orderNo, amount, eventId);
	}

	private Payment process(String orderNo, BigDecimal amount, UUID eventId) {
		...
		// 결제창이 없으므로 우리가 발급해서 PG 에 보낸다.
		String paymentId = "PAY-" + UUID.randomUUID();
		PgConfirmResult result = pgClient.confirm(orderNo, paymentId, amount);
