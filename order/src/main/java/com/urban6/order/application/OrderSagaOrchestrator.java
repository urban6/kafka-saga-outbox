 ...
 * <p>
 * 이 클래스가 하는 일의 대부분은 <b>안 하는 것</b>이다. at-least-once 라 같은 회신이 두 번 오고,
...
public class OrderSagaOrchestrator {
	...
	/**
	 * 회신을 받았을 때 무엇을 할지. {@link #decide} 의 반환값이자 이 사가가 가질 수 있는 결정의 전부다.
	...
	/**
	 * <b>순수 함수.</b> 현재 단계와 회신 타입만 보고 결정한다. DB·시계·랜덤을 건드리지 않으므로
	 ...
	 * <p>
	 * 사가 전이를 먼저 하는 이유는 그게 <b>최종 방어선</b>이기 때문이다. 방어선 3은 읽고 나서
	...
	private void apply(SagaInstance saga, SagaDecision decision) {
		...
		int orderMoved = orderRepository.transitionStatus(
				orderNo, OrderStatus.CREATED, nextOrderStatus, now);
		if (orderMoved == 0) {
			// 사가는 STARTED 였는데 주문은 CREATED 가 아니다 = 둘이 어긋났다.
			// 조용히 넘어가면 재고만 움직이고 주문은 그대로 남는다. 롤백시켜야 한다.
			throw new IllegalStateException(
					"order status mismatch. orderNo=" + orderNo + " expected=CREATED");
		}
