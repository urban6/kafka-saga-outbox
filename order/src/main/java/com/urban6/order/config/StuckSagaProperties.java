 ...
 * <p>
 * 계산 메서드는 전부 순수 함수다. {@code OrderSagaOrchestrator.decide()} 와 같은 성격으로,
 * 스프링 없이 전 조합을 단위 테스트할 수 있다.
 *
 * @param scanInterval     스캔 주기. 이 값은 자바에서 읽지 않고 {@code @Scheduled} 가
 *                         {@code ${saga.stuck.scan-interval}} 로 직접 읽는다.
...
public record StuckSagaProperties(
	...
	/**
	 * 후보 조회에 쓸 컷. <b>이보다 어린 사가는 어떤 단계여도 stuck 이 아니다.</b>
	 * <p>
	 * SQL 은 {@code step_started_at} 하나로만 자를 수 있는데 임계값은 단계마다 다르다.
