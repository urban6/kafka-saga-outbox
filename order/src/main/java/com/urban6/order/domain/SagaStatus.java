 ...
 *
 * OrderStatus 는 고객에게 보여줄 주문의 상태고, 이건 오케스트레이션의 진행 상태다.
 * Stuck Saga 탐지는 idx_stuck (status, step_started_at) 으로 이 컬럼만 보고 돈다 —
...
public enum SagaStatus {
	 ...
	 *
	 * 진행 중은 STARTED 하나뿐이라 여집합으로 봐도 된다. 보상 상태를 두지 않은 이유는
	 * 보상이 재고 해제와 주문 취소뿐이고 둘 다 로컬이라 같은 트랜잭션에서 끝나기 때문이다 —
