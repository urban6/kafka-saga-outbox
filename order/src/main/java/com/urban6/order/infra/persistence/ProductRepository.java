 ...
 * {@code 0} 이면 조건이 안 맞은 것이니 호출부가 그에 맞게 분기한다.
 * <p>
 * {@code flushAutomatically = true} 는 벌크 연산 전에 영속성 컨텍스트를 먼저 밀어내라는 뜻이다.
 * 안 그러면 아직 flush 안 된 변경이 UPDATE 뒤에 덮어써서 순서가 뒤집힌다.
 * {@code clearAutomatically} 는 켜지 않는다 — 컨텍스트를 비우면 같은 트랜잭션에서 만들고 있던
 * {@code Order} 까지 detach 돼서 이후 변경이 유실된다. 대신 <b>이 메서드들을 호출한 뒤에는
 * 같은 트랜잭션에서 {@code Product} 를 엔티티로 다시 읽지 않는다</b>(읽으면 1차 캐시의 옛 값이 나온다).
 */
public interface ProductRepository extends JpaRepository<Product, String> {
     ...
     * <p>
     * "조회해서 재고를 확인하고 예약한다" 가 아니라 <b>확인과 예약이 한 문장</b>이다.
     ...
     * <p>
     * {@code reservedQuantity >= :quantity} 조건은 중복 확정으로 음수가 되는 걸 막는다.
