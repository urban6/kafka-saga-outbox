...
public interface SagaInstanceRepository extends JpaRepository<SagaInstance, UUID> {
	...
	/**
	 * 사가 상태 전이. step_started_at 은 건드리지 않는다 —
	...
	/**
	 * Stuck 후보. 최종 판정은 호출자가 단계별 임계값으로 한다 —
	 * SQL 은 step_started_at 하나로만 자를 수 있는데 임계값은 단계마다 다르기 때문이다.
	 * 가장 짧은 임계값으로 넉넉히 뽑고 나머지는 메모리에서 거른다.
	 *
	 * statuses 는 비종료 상태 전부(isTerminated() 의 여집합)를 받는다.
	 * 지금은 STARTED 하나지만 컬렉션으로 받는다 — 상태가 늘었을 때 여기에 추가하는 걸 잊으면
	 * 사가가 멈췄는데 아무도 모르는 상태가 된다.
	 *
	 * idx_stuck (status, step_started_at) 을 탄다. IN 이라 상태값마다 레인지 스캔이
	 * 나뉘고 정렬이 한 번 더 붙지만, Pageable 로 잘라 읽으므로 대상이 커져도 비용이 고정된다.
	 *
	 * 가장 오래된 행이 항상 첫 번째라는 게 중요하다. 잘려도 "가장 오래된 정체의 나이"는 정확하다.
	 */
	@Query("""
			select s
			  from SagaInstance s
